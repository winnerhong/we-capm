...
export interface TopAcornFamily {
  ...
  acorns: number;
  /**
   * 등수 점수. 도토리와 다른 값이다 — 도토리는 지갑, 점수는 순위다.
   *
   * 도토리는 미션마다 고정 정수라 다 한 집은 전부 동점이 됐다. 점수는
   * 「도토리×100 + 속도 보너스 − 반려 감점」이고, 속도가 초 단위 연속량이라
   * 1초만 달라도 갈린다. 계산식은 lib/scoring/core.ts.
   *
   * null 이면 아직 점수 원장이 없다(마이그레이션 20260902000000 적용 전).
   * 그때는 예전처럼 도토리 합계로 줄을 세운다.
   */
  score?: number | null;
}
