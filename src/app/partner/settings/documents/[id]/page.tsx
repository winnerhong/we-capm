...
export default async function DocumentDetailPage({ params }: PageProps) {
    ...
    return (
      <div className="space-y-4 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-center">
        ...
        <p className="text-sm text-rose-700">
          서류 조회는 <b>OWNER</b> 또는 <b>FINANCE</b> 역할만 이용할 수 있어요.
        </p>
      </div>
    );
  }
  ...
  if (!doc) notFound();
  // 소유권 확인
  if (doc.partner_id !== session.id) {
    return (
      <div className="space-y-4 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-center">
        <h1 className="text-xl font-bold text-rose-800">접근 불가</h1>
        <p className="text-sm text-rose-700">이 서류에 접근할 권한이 없어요.</p>
        <Link
  ...
  // 버전 히스토리: 같은 partner_id + doc_type의 이전 버전
  const allDocs = await loadPartnerDocuments(session.id);
  const history = allDocs
    .filter(
      (d) => d.doc_type === doc.doc_type && d.id !== doc.id
    )
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
        ...
        <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
          <Info label="상태" value={`${statusMeta.icon} ${statusMeta.label}`} />
            ...
            {history.map((h) => {
              ...
              return (
                <li key={h.id} className="flex items-center gap-2 py-2">
                  ...
                  <Link
                    href={`/partner/settings/documents/${h.id}`}
      ...
      <section className="flex flex-wrap items-center gap-2 rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
        ...
        <Link
          href={`/partner/settings/documents/upload?type=${doc.doc_type}`}
