...
type TierDraft = {
  _uid: string;
  label: string;
  threshold: string; // editing state as string
  reward_desc: string;
  icon: string;
};
...
function parseConfig(raw: Record<string, unknown>): FinalRewardMissionConfig {
  ...
  const redemption_ttl_hours =
    typeof raw.redemption_ttl_hours === "number" ? raw.redemption_ttl_hours : 24;
...
export function FinalRewardMissionEditor({ mission }: Props) {
  ...
  function validate(): string | null {
    ...
    if (!Number.isFinite(ttl) || ttl <= 0) {
      return "교환권 유효시간(시)을 올바르게 입력해 주세요";
    }
    return null;
  }
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="기본 도토리 (지급 아님)" htmlFor="default_acorns_fixed">
                <input
                  id="default_acorns_fixed"
                  type="number"
                  value={0}
                  disabled
                  readOnly
        ...
        <section className="rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-[11px] text-[#8B7F75]">
            제출 시 누적 도토리 기준으로 가장 높은 달성 티어가 자동 선택돼요.
            정렬은 달성 도토리 오름차순으로 저장됩니다.
          </p>
        </section>
        ...
        <section className="rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
          ...
          <div className="space-y-4">
            <Field label="교환권 유효시간 (시간)" htmlFor="ttl_hours">
              ...
              />
              <p className="mt-1 text-[11px] text-[#8B7F75]">
                발급 후 이 시간이 지나면 QR이 만료됩니다. 기본 24시간.
              </p>
            </Field>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-[#6B6560]">
                    🎁 최종 보상 · {scope === "QUEST_PACK" ? "팩 단위" : "전체 팩"}
                  </p>
                </div>
              </div>
