...
// 템플릿 카드의 [보관]/[복원]/[삭제] 액션 — list page 인라인 사용.
...
} from "@/lib/timetable-templates/actions";
...
export function TemplateRowActions({ id, label, archived }: Props) {
  ...
  const onArchive = () => {
    startTransition(async () => {
      try {
        await archiveTimetableTemplateAction(id, !archived);
        router.refresh();
      } catch (e) {
  ...
  const onDelete = () => {
    if (!window.confirm(`"${label}" 템플릿을 영구 삭제할까요?`)) return;
    startTransition(async () => {
      try {
        await deleteTimetableTemplateAction(id);
        router.refresh();
      } catch (e) {
