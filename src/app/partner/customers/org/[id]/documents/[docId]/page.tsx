...
import { deleteOrgDocumentByPartnerAction } from "../actions";
import { OrgDocReviewActions } from "./review-actions";
...
async function loadOrgName(id: string): Promise<string | null> {
    ...
    supabase.from("partner_orgs" as never) as unknown as {
      select: (c: string) => {
        eq: (k: string, v: string) => {
          maybeSingle: () => Promise<{
            data: { org_name: string; partner_id: string | null } | null;
          }>;
        };
      };
    }
...
export default async function PartnerOrgDocDetailPage({ params }: PageProps) {
    ...
    return (
      <div className="space-y-4 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-center">
        ...
        <p className="text-sm text-rose-700">
          서류 조회는 <b>OWNER</b> 또는 <b>FINANCE</b> 역할만 가능해요.
        </p>
      </div>
    );
  }
  ...
  if (!orgRow) notFound();
  if (!orgRow.partner_id || orgRow.partner_id !== session.id) {
    return (
      <div className="space-y-4 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-center">
        <h1 className="text-xl font-bold text-rose-800">접근 불가</h1>
        <p className="text-sm text-rose-700">
          이 기관 서류에 접근할 권한이 없어요.
        </p>
      </div>
    );
  }

  const doc = await loadOrgDocumentById(docId);
  if (!doc) notFound();
  if (doc.org_id !== orgId) {
    return (
      <div className="space-y-4 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-center">
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-xs text-[#6B6560] md:text-sm">
              {orgRow.org_name} · 버전 v{doc.version} ·{" "}
              {fmtDateTime(doc.submitted_at)}
            </p>
          </div>
          <span
      ...
      {doc.status === "PENDING" && (
        <section className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-5 shadow-sm">
          ...
          <p className="mb-3 text-xs text-amber-800">
            파일을 확인하고 승인 또는 반려해 주세요. 반려 시 기관에게 사유가
            전달돼요.
          </p>
          <OrgDocReviewActions orgId={orgId} documentId={doc.id} />
        </section>
      )}
            ...
            {history.map((h) => {
              ...
              return (
                ...
                >
                  ...
                  <Link
                    href={`/partner/customers/org/${orgId}/documents/${h.id}`}
      ...
      <section className="flex flex-wrap items-center gap-2 rounded-2xl border border-[#D4E4BC] bg-white p-5 shadow-sm">
        ...
        <Link
          href={`/partner/customers/org/${orgId}/documents/upload?type=${doc.doc_type}`}
        ...
        {session.role === "OWNER" && (
          <form
            action={async () => {
              "use server";
              await deleteOrgDocumentByPartnerAction(orgId, doc.id);
            }}
            className="ml-auto"
          >
            <button
