...
import { createSingleEventParticipantAction } from "@/app/org/[orgId]/events/[eventId]/users/actions";
...
export default async function NewOrgUserPage({
  ...
  const baseHref = `/org/${orgId}/users/new`;

  // 폼 액션 — 행사 선택 시 자동 연결
  const singleAction = selectedEvent
    ? createSingleEventParticipantAction.bind(
        null,
        orgId,
        selectedEvent.event_id
      )
    : undefined; // 미선택 시 NewUserForm 의 기본 액션 사용

  // 일괄 등록 액션 — 행사 선택 시 같이 binding
  const bulkAction = bulkImportAppUsersAction.bind(
    null,
    orgId,
        ...
        >
          <div className="mx-auto flex max-w-3xl items-center gap-2 px-4 py-2 text-xs font-semibold">
            ...
            <span className="truncate">
              <b>{selectedEvent.name || "(이름 없음)"}</b> 행사에 등록 중
            </span>
            <Link
              href={baseHref}
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-6">
          ...
          <header className="relative overflow-hidden rounded-3xl border-2 border-emerald-300 shadow-md">
            ...
            <div className="relative z-[1] p-5 text-white md:p-6">
              ...
              <h1 className="mt-3 text-xl font-extrabold leading-tight drop-shadow-md md:text-2xl">
                🙋 {selectedEvent.name || "(이름 없음)"} — 참가자 한 명 등록
              </h1>
              <p className="mt-1 text-xs text-white/95 drop-shadow md:text-sm">
                여기서 등록하는 참가자는 이 행사 명단에 자동 연결돼요.
              </p>
            </div>
          </header>
        ) : (
        ...
        {selectedEvent ? (
          <NewUserForm
            orgId={orgId}
            action={singleAction}
            submitLabel={`🌱 "${selectedEvent.name || "(이름 없음)"}" 행사에 등록`}
          />
        ) : (
          <section
            aria-label="행사 선택 필요"
          ...
          >
            ...
            <p className="mt-1 text-xs text-[#6B6560]">
              ...
              <span className="font-bold text-[#2D5A3D]">🎪 어디에 등록할까요?</span>
              {" "}영역에서 행사를 골라 주세요.
            </p>
            {events.length === 0 && (
              <Link
        ...
        <details className="group" open={false}>
          ...
          <div className="space-y-4 border-t border-[#D4E4BC] p-5 md:p-6">
            ...
            {selectedEvent ? (
              ...
              <div className="rounded-2xl border-2 border-dashed border-[#E5D3B8] bg-[#FFFDF8] px-5 py-6 text-center">
                ...
                <p className="mt-1 text-[11px] text-[#6B6560]">
                  위쪽 행사 칩에서 등록할 행사를 고르면 일괄 등록이 활성화됩니다.
                </p>
              </div>
            )}
          </div>
