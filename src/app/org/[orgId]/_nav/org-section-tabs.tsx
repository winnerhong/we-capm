...
const SECTIONS: {
  ...
  //            글자 하나 다르지 않다.
  //   참가자 — 행사 안 [참가자] 탭이 목록·검색·출석·도토리는 물론 비활성화·
  //            영구삭제·기관에서 빼기까지 **같은 UserRowActions 컴포넌트**로
  //            처리한다. 행사에서 빼기는 거기에만 있다. 게다가 이 화면의 빠른
  //            원생 추가와 엑셀 일괄 등록은 행사를 고르기 전엔 눌리지도 않았다 —
  //            행사를 골라야 쓰는 기능이면 애초에 행사 안에 있어야 한다.
  //
  // 행사 하나를 준비하는 일은 행사 안에서 끝난다 — 이 줄은 "어느 행사?" 만
  // 고른다. 뺀 화면들은 남아 있고, 기관 홈 「모든 기능」이 그 길이다:
  //   [✉️ 초대장 템플릿] — 기관 단위 자산이라 행사가 없어도 만들 수 있어야 한다
  //   [👨‍👩‍👧 참가자]     — CSV 전체 내보내기·참가자 로그인 링크·가족 명단 가는
  //                       길이 거기에만 있고, 행사가 하나도 없는 기관에는
  //                       참가자를 볼 다른 화면이 없다
