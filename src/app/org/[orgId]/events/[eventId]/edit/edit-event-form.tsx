...
export function EditEventForm({
  ...
  const [invEntryLead, setInvEntryLead] = useState(
    initial.invitation_entry_lead_min
  );
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.set("invitation_entry_lead_min", invEntryLead.trim());
  ...
  return (
    ...
    >
      {/* 넓은 화면에선 폼 **왼쪽**에 붙어 스크롤을 따라오고, 좁은 화면에선
          폼 위의 버튼 하나로 접힌다. 폰으로 폼을 채우면서 옆에 폰을 또 띄울 순 없다.
          JSX 에서 미리보기가 폼보다 먼저 오는 이유가 이것이다 — order 로 뒤집지
          않는다. 뒤집으면 탭 이동 순서(폼 → 미리보기)가 눈에 보이는 순서와 어긋난다. */}
      {mode === "invitation" && (
