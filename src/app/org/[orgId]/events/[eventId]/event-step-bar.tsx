              ...
              <li key={s.key} className="flex">
                ...
                >
                  ...
                  {/* 상태 한 단어 — 설명문 대신 이 자리가 다음 할 일을 말한다.
                      ...
                      10px 이 아니라 11px 인 이유: "스탬프북 없음" 이 "ㅅ탬프북
                      없음" 으로 보인다는 신고를 받았다. 스 는 ㅅ 과 ㅡ 뿐이라
                      가로획 하나가 사라지면 정확히 ㅅ 만 남는다. 10px 에서 그
                      획은 1px 이 안 돼 픽셀 사이에 끼고, 안티에일리어싱이
                      배경으로 지워 버린다. 색도 함께 올렸다(3.75:1 → 5.5:1).
                      한글은 한 글자에 자모가 두셋 들어가므로 같은 px 이라도
                      라틴 문자보다 획이 얇다. 10px 은 한글에 쓰면 안 된다. */}
                  <span className="pl-[1.375rem] text-[11px] text-[#6B6560]">
                    {st.hint ?? (done ? "완료" : "")}
                  </span>
                </Link>
              </li>
