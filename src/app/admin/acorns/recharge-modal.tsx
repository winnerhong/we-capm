        ...
        >
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 id="recharge-modal-title" className="text-lg font-bold text-[#2D5A3D]">
                🌰 도토리 충전
              </h2>
          ...
          {success ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="mt-2 text-sm text-[#6B6560]">
                ...
                <br />
                🌰 <b>{success.units.toLocaleString("ko-KR")}</b>개가 지급되었어요
              </p>
              <p className="mt-2 text-xs text-[#8B6F47]">
                현재 잔액: 🌰 {success.balance.toLocaleString("ko-KR")}
              </p>
            ...
            <>
                  ...
                  >
                    {partners.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name}
                        {p.business_name ? ` (${p.business_name})` : ""} · 현재 🌰{" "}
              ...
              <div className="rounded-xl bg-[#FFF8F0] border border-[#D4E4BC] p-4 mb-4 space-y-1 text-sm">
                ...
                <div className="flex justify-between text-[#6B6560]">
                  <span>기본 지급</span>
                  <span>🌰 {tier.baseUnits.toLocaleString("ko-KR")}</span>
                </div>
                {bonusAcorns > 0 && (
                  <div className="flex justify-between text-[#C4956A] font-semibold">
                    <span>보너스 (+{tier.bonusPct}%)</span>
                    <span>🌰 +{bonusAcorns.toLocaleString("ko-KR")}</span>
                  </div>
                )}
